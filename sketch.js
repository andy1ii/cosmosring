...
function setupGadget() {
  ...
  styleUIElement(pillInput); 
  pillInput.style('width', '120px');
...
function drawLogoMode() {
  ...
  if (!isTicking) rotationOffset = frameCount * 0.03;
      ...
      let tickIndex = floor(frameCount / period), t = constrain((frameCount % period) / (period * 0.5), 0, 1);
      rotationOffset = (tickIndex + (1 - pow(1 - t, 3))) * stepSize;
  }
...
function positionUI() {
  ...
  gadgetContainer.style('left', '50%'); gadgetContainer.style('top', '50%');
